 ...
 *
 * \brief Fetch message data from the server.
 *
...
public:
    /*!
     * \class KIMAP::FetchJob::FetchScope
     * \inmodule KIMAP
     * \inheaderfile KIMAP/FetchJob
     *
     * \brief Used to indicate what message data should be fetched.
     *
    ...
    public:
         ...
         *
         * \value Headers Fetch RFC-2822 or MIME message headers.
         ...
         *                - The message UID
         *
         * \value Flags Fetch the message flags (the UID is also fetched)
         * \value Structure Fetch the MIME message body structure (the UID is also fetched)
         * \value Content Fetch the message content (the UID is also fetched). To fetch only certain MIME parts (see Structure), populate the parts field.
         * \value Full Fetch the complete message.
         * \value HeaderAndContent Fetch the message MIME headers and the content of parts specified in the parts
         * field. If parts is empty, this mode will return the full message, just like
         * FetchScope::Content. Use case: - Start a FetchJob with the FetchScope::Structure mode to retrieve the structure of the message - Parse the structure
         * to identify the parts that are interesting (ie: probably everything but attachments) - Start another FetchJob with FetchScope::HeaderAndContent to
     ...
     *
     * Returns \c true if the result of sequenceSet() should be
     *          interpreted as UIDs, \c false if it should be interpreted
     *          as sequence numbers
     */
...
Q_SIGNALS:
     ...
     * bee enabled on the server, and FetchScope::qresync has been set to \c true.
     * It contains a list of messages that have vanished from the mailbox since the
     * last modification sequence specified in FetchScope::changedSince.
     *
